...
public class GameScreen extends GLScreen {
	...
	// Esta cámara se usa solo para definir la resolucion de las pantallas
	// de interacción con el usuario, como menú pausa, ready, etc.
	Camera2D guiCam;
	Vector2 touchPoint;
	...
	public GameScreen(Game game) {
		...
		worldListener = new WorldListener() {
			...
			public void speedUpBackground(float velocity) {
				Assets.speedUpBackgroundAnimation(velocity);
			}
		};
		world = new World(worldListener, level);
		renderer = new WorldRenderer(glGraphics, batcher, world);
		// Acá todo es en pixels
	...
	private void updateRunning(float deltaTime) {
		...
		if (world.state == World.WORLD_STATE_NEXT_LEVEL) {
			// Se setea calculate porque se llegó a la casa y no se ejecutó
			// el evento UP.
			calculate = false;
	...
	private void updateLevelEnd() {
		/* 
		 * Si el acelerómetro no está activo, ignoro el primer TOUCH_UP ya que cuando se llega
		 * a la casa, el usuario aún tiene el dedo sobre la pantalla.
		 */
		List<TouchEvent> touchEvents = game.getInput().getTouchEvents();
		int len = touchEvents.size();
		for (int i = 0; i < len; i++) {
