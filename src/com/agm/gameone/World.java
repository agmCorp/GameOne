...
public class World {
	public interface WorldListener {
		...
		public void speedUpBackground(float velocity);
	}

	public static final float WORLD_WIDTH = 10;
	// 16 PANTALLAS
	public static final float WORLD_HEIGHT = 15 * 16;
	...
	public World(WorldListener listener, int level) {
		...
		// Camara
		this.birdsEyeView = new BirdsEyeView(WORLD_WIDTH / 2, BirdsEyeView.BIRDSEYEVIEW_HEIGHT / 2, level - 1);
		listener.speedUpBackground(this.birdsEyeView.velocity.y);
	...
	private void generateLevel(int level) {
		 ...
		 * Se inicia por sobre el Héroe.
		 */
		float y = 22;
		float x;
		
		/*
		 * Ecuación de la recta que pasa por A(1, 0.93) y B(MAX_LEVEL, 0).
		 * Siendo 0.93 la probabilidad elegida de generación de enemigos para level 1.
		 */
	...
	/*
	 * Si accelEnabled = true: recibe lectura de acelerómetro.
	...
	/* Si accelEnabled = true: recibe lectura de acelerómetro 
	 * Si accelEnabled = false: recibe la cantidad de metros que debe incrementarse a la posición 
	 * del Hero. Para mover al héroe debo asignar su nueva velocidad, por lo tanto se asignan los 
	...
	private void updateHero(float deltaTime, float dX, float dY, boolean accelEnabled) {
		if (hero.state != Hero.HERO_STATE_HIT) { 
			if (!accelEnabled) {
				...
				// Normaliza los valores del acelerómetro de [-10, 10] a [-1, 1]
